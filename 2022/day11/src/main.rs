...
fn main() {
        ...
        match info_words.next() { 
            ...
            Some("Starting") => { 
                for s in halves.next().unwrap().split(",") {
                    let item = s.trim().parse::<u64>().unwrap();
            ...
            Some("Test") => { 
                ...
                let id = monkeys.len() - 1;
                let mut current_monkey = monkeys.get_mut(id).unwrap();
                current_monkey.divisible_by = words.next().unwrap().parse::<u64>().unwrap();

                common_divisor *= current_monkey.divisible_by;
            },
    ...
    let mut most_busy = (12341324, 0u64);
    let mut secondmost_busy = (12341234, 0u64);
    ...
    for _round_id in 0..(if PART_B { 10000 } else { 20 }) {
...
//                println!("  Monkey inspects an item with a worry level of {}.", item);
                ...
                let value: u64;
                if let Ok(x) = op_words.next().unwrap().parse::<u64>() {
                    value = x;
                } else {
                    value = item;
                }
                ...
                match op {
                    Some("+") => { item = item + value; /*println!("    Worry level increasese by {value} to {item}."); */ },
                    Some("*") => { item = item * value; /*println!("    Worry level is multiplied by {value} to {item}.");*/  },
                    Some("-") => { item = item - value; /*println!("    Worry level decreases by {value} to {item}.");*/  },
                    Some("/") => { item = item / value; /*println!("    Worry level is divided by by {value} to {item}."); */ },
                ...
                if (item % src_monkey.divisible_by) == 0 {
//                    println!("    Current worry level is divisible by {}", src_monkey.divisible_by);
                    ...
                    dest_monkey.items.push_back(item);
...
//        println!("After round {_round_id}, the monkeys are holding items with these worry levels:");
