...
impl Map {
    ...
    fn compute_distances(&mut self) {
        // implement floyd-warshall to create a map of shortest distances between two nodes
...
fn main() {
    ...
    let best = if PART_B {
        best_flow(&mut map, start_room_id, start_room_id, 0, &mut valve_rooms, 26, 0, 0)
    } else {
        best_flow(&mut map, start_room_id, start_room_id, 0, &mut valve_rooms, 30, 0, u32::MAX)
    };
